...
$remote_command = $ScriptRoot + "\rmfile.txt"
$total_errors = 0
$total_vmhosts = 0
$index_vmhosts =0
	...
	if ($error.Count -eq 0){
		...
		if ($vmhosts_in_cluster) {
			...
			foreach ($vmhost in $vmhosts_in_cluster) {
				write-progress -Activity "Removing unneeded script for $Location container" -Status "Percent complete $("{0:N2}" -f (($index_vmhosts / $total_vmhosts) * 100))%" -PercentComplete (($index_vmhosts / $total_vmhosts) * 100) -CurrentOperation "Processing vSphere host: $($vmhost.name)"	
